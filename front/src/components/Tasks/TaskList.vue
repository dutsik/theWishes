<template>
  <div>
    ...
    <b-nav pills class="py-2 justify-content-center shadow-sm">
      <b-nav-item  :class="[status === 'active' ? 'pill-active' : 'pill-inactive']" class="pill-default lg:flex-no-grow no-underline" :to="{name: 'TaskList', params: { status: 'active' }}" exact>
          Active ({{ activeTasks.length }})
      </b-nav-item>
      <b-nav-item :class="[status === 'completed' ? 'pill-active' : 'pill-inactive']" class="pill-default lg:flex-no-grow no-underline" :to="{name: 'TaskList', params: { status: 'completed' }}" exact>
